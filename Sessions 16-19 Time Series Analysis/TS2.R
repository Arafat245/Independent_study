...
# Try ar(1) p=1 because PACF first becomes insignificant after lag 1
...
# Try ma(2) p=0, q=2 because ACF first becomes insignificant after lag 2
...
# Try arma(1,2) p=1, q=2 because both ACF and PACF are sinusoidal
# so there are probably AR and MA terms
...
# To automatically select models, we will use the auto.arima() function, 
# which finds the order (e.g. (p,q)) that minimizes the AIC
# We will learn what the "I" stands for next lecture
...
temp.arima111 <- arima(e.ts.temp, order=c(1,1,1))
summary(temp.arima111)

temp.arima111 <- arma(diff(e.ts.temp), order=c(1,1), include.intercept=FALSE)
summary(temp.arima111)
