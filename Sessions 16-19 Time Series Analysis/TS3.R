...
model1.predictions <- ggplot() + 
  ...
  geom_line(aes(x=time.predictions,y=E_Y.pred + e_t.pred$upper[,2]),
            color="red",linetype="dashed") +
  xlab("") + ylab("Richmond Tmin") + 
  ggtitle("Tmin Trend and Seasonality Model + ARIMA of Residuals")
model1.predictions
...
set.seed(1)
temp.resid.sim <- arima.sim(n=12*50, list(ar=c(temp.auto$coef[1],temp.auto$coef[2]),
...
temp.50.yr.mean <- predict(temp.trend.seasonal, newdata=time.50yr.df)

# plot simulated temperatures
autoplot(ts(temp.50.yr.mean + temp.resid.sim),xlab="Time",ylab="Simulated Richmond Tmin")
